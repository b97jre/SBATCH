...
public class ShrimpSBATCH {
	...
	public void shrimpDir(ExtendedWriter generalSbatchScript,
			SBATCHinfo sbatch, String timestamp) {
			...
			try {
				...
				for (int i = 0; i < fileNames.size(); i++) {
					EW.println("gmapper-cs "
							...
							+ inDir + "/" + fileNames.get(i) + " "
							+ this.referenceFile + ">" + outDir + "/"
							+ fileNames.get(i) + "_" + refName + ".sam 2>"
							+ outDir + "/" + fileNames.get(i) + "_" + refName
							+ ".sam.log");
					if ((i + 1) % 8 == 0 && fileNames.size() - i != 1) {
						...
						sbatchFileName = inDir + "/scripts/" + timestamp + "_"
								+ count + ".SHRIMP.sbatch";
						generalSbatchScript.println("sbatch " + sbatchFileName);
