...
public class FilterFastqSBATCH {
	...
	public void run(Hashtable<String, String> T) {
		...
		if (T.containsKey("-codeDir"))
			...
			System.out
					.println("must contain the directory of filter_fastq.pl -codeDir");
			allPresent = false;
		}
	...
	public void filter_fastqSample(ExtendedWriter generalSbatchScript,
			SBATCHinfo sbatch, String sample, String timestamp, int count2) {
		...
		if (fileNames.isEmpty()) {
			...
			try {
				String sbatchFileName = finalOutDir + "/scripts/" + timestamp
						+ ".sbatch";
				...
				for (int i = 0; i < pairs.size(); i++) {
					...
					EW.print(codeDir + "/filter_fastq.pl " + appendix + " "
							+ finalOutDir + " ");
