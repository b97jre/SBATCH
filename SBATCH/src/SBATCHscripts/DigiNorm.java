...
public class DigiNorm {
	...
	public void run(Hashtable<String, String> T) {
		...
		codeFile = Functions.getValue(T, "-codeFile",
				"~/bin/khmer/scripts/normalize-by-median.py");
	...
	public void DigiNormTop(SBATCHinfo sbatch, String timeStamp,
			String projectDir, String inDir, String outDir) {
		try {
			...
			ExtendedWriter EW = new ExtendedWriter(new FileWriter(projectDir
					+ "/scripts/" + timeStamp + "_filter_fastq.sh"));
			DigiNormDir(EW, sbatch, projectDir + "/" + inDir, projectDir + "/"
					+ outDir, timeStamp, 0);
			EW.flush();
			EW.close();
		} catch (Exception E) {
	...
	public void DigiNormDir(ExtendedWriter generalSbatchScript,
			SBATCHinfo sbatch, String inDir, String outDir, String timestamp,
			int count2) {
		...
		for (int i = 0; i < samples.size(); i++) {
			DigiNormDir(generalSbatchScript, sbatch,
					inDir + "/" + samples.get(i),
					outDir + "/" + samples.get(i), timestamp, i);
		}
		...
		if (fileNames.isEmpty()) {
			...
			try {
				...
				if (suffix.indexOf("gz") > -1)
					EW.println("zcat *." + suffix + " | gzip > " + outDir
							+ "/all." + suffix);
				else {
					EW.println("cat *." + suffix + " | gzip > " + outDir
							+ "/all." + suffix + ".gz");
					outFile = "all." + suffix + ".gz";
				}
				EW.println("cd " + outDir);
				EW.println(codeFile + " -C " + C + " -k " + k + " -N " + N
						+ " -x " + x + " " + outFile);
				EW.println();

				EW.println("python ~/bin/khmer/sandbox/split-pe.py " + outFile
						+ ".keep");
				EW.println("rm " + outFile);
				EW.println("rm " + outFile + ".keep");
				EW.println("mv " + outFile + ".keep.1 " + outFile
						+ ".keep.1.fa");
				EW.println("mv " + outFile + ".keep.2 " + outFile
						+ ".keep.2.fa");
