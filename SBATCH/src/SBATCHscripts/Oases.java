...
public class Oases {
	...
	public void setValues(int maxKmer, int minKmer, int step, int mergeKmer,
			String suffix, int insertLength, String projectDir,
			boolean strandSpecific) {
	...
	public void oasesFilePE(ExtendedWriter EW, ArrayList<String[]> pairs,
			String outDir, String memory, int CPUs) {
		// oases.pl --seqType fq --left 13_2/QC/13_2_1.fastq --right
		// 13_2/QC/13_2_2.fastq --output
		// /bubo/proj/b2011098/private/oases/13_2/output --min_contig_length 200
		// --CPU 10 --bflyHeapSpace 10000M
		...
		if (this.strandSpecific)
			oasescommand += oasescommand += " -d \"-" + this.suffix
					+ " -strand_specific -separate left." + this.suffix
					+ " right." + this.suffix + " \"";
		else
			oasescommand += " -d \"-" + this.suffix + " -separate left."
					+ this.suffix + " right." + this.suffix + " \"";

		EW.println(oasescommand);

	}

	public void oasesFileSR(ExtendedWriter EW, ArrayList<String> files,
			String outDir, String memory, int CPUs) {
		// oases.pl --seqType fq --left 13_2/QC/13_2_1.fastq --right
		// 13_2/QC/13_2_2.fastq --output
		// /bubo/proj/b2011098/private/oases/13_2/output --min_contig_length 200
		...
		if (this.strandSpecific)
			oasescommand += oasescommand += " -d \"-" + this.suffix
					+ " -strand_specific single." + this.suffix + " \"";
		else
			oasescommand += oasescommand += " -d \"-" + this.suffix
					+ " single." + this.suffix + " \"";
	...
	public static String FileStart(int nrOfSequences, String inDir,
	...
	// public void dirPE( ExtendedWriter generalSbatchScript, SBATCHinfo sbatch
	...
	// oasesDirPE(generalSbatchScript,sbatch,timestamp,inDir+"/"+subDirs.get(i),outDir+"/"+subDirs.get(i),
	// insertSize);
	// }
	// }
	//
	//
	// public void oasesDirSR(ExtendedWriter generalSbatchScript, SBATCHinfo
	// sbatch ,String timestamp,String inDir, String outDir ){
	//
	// ArrayList <String> fileNames = IOTools.getSequenceFiles(inDir,suffix);
	// if(fileNames.isEmpty()){
	// System.out.println("No "+suffix+" files in folder :"+inDir);
	...
	//
	// String memory = oasesFileStart(nrOfSequences, inDir, outDir, timestamp,
	// count, sbatch, EW,time );
	//
	// EW.println();
	// EW.println();
	...
	// }
	//
	// public void oasesDirSRSingle( ExtendedWriter generalSbatchScript,
