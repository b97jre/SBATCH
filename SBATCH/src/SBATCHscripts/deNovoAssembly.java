...
public class deNovoAssembly {
	...
	public void run(Hashtable<String, String> T) {
		...
		if (allPresent) {
			deNovoScript(sbatch, timeStamp, projectDir + "/" + inDir,
					projectDir + "/" + trinityDir, projectDir + "/" + oasesDir);
		} else
			System.out
					.println("\n\nAborting run because of missing arguments for trinity.");
	}

	public void deNovoScript(SBATCHinfo sbatch, String timeStamp, String inDir,
			String trinityDir, String oasesDir) {
		try {
			...
			if (this.pairedEnd) {
				if (!interactive)
					EW = new ExtendedWriter(new FileWriter(projectDir
							+ "/scripts/" + timeStamp + "_PE_deNovo.sh"));
				else
					EW = new ExtendedWriter(new FileWriter(projectDir
							+ "/scripts/PE_deNovo.sh"));
				...
				if (!interactive)
					System.out.println("sh " + projectDir + "/scripts/"
							+ timeStamp + "_PE_deNovo.sh > " + projectDir
							+ "/scripts/" + timeStamp + "_PE_deNovo.sh.out 2>"
							+ projectDir + "/scripts/" + timeStamp
							+ "_PE_deNovo.sh.err");
				else
					System.out.println("sh " + projectDir
							+ "/scripts/PE_deNovo.sh > " + projectDir
							+ "/scripts/PE_deNovo.sh.out 2>" + projectDir
							+ "/scripts/PE_deNovo.sh.err");

			} else {
				if (!interactive)
					EW = new ExtendedWriter(new FileWriter(projectDir
							+ "/scripts/" + timeStamp + "_SR_deNovo.sh"));
				else
					EW = new ExtendedWriter(new FileWriter(projectDir
							+ "/scripts/SR_deNovo.sh"));
				...
				if (!interactive)
					System.out.println("sh " + projectDir + "/scripts/"
							+ timeStamp + "_SR_deNovo.sh >" + projectDir
							+ "/scripts/" + timeStamp + "_SR_deNovo.sh.out 2>"
							+ projectDir + "/scripts/" + timeStamp
							+ "_SR_deNovo.sh.error");
				if (interactive)
					System.out.println("sh " + projectDir
							+ "/scripts/SR_deNovo.sh >" + projectDir
							+ "/scripts/SR_deNovo.sh.out 2>" + projectDir
							+ "/scripts/SR_deNovo.sh.error");
			}
	...
	public void deNovoDirPE(ExtendedWriter generalSbatchScript,
		...
		if (!pairs.isEmpty() && (oases || trinity)) {
			System.out
					.println("creating de novo shell scripts for paired reads in "
							+ inDir);
			int count = 0;
			int nrOfSequences = 0;
			if (!interactive)
			...
			if (trinity) {
				try {
					...
					if (!interactive) {
						sbatchFileName = trinityDir + "/scripts/" + timestamp
								+ "_" + count + "_trinity.sbatch";
						generalSbatchScript.println("sbatch " + sbatchFileName);
					} else {
						sbatchFileName = trinityDir + "/scripts/" + count
								+ "_trinity.sh";
					...
					if (interactive) {
						...
						memory = Trinity.trinityFileStart(nrOfSequences, inDir,
								trinityDir, timestamp, count, sbatch, EW, time);
						if (memory.compareTo("2G") == 0)
							CPUs = 2;
					}
					...
					run.trinityFilePE(EW, pairs, trinityDir, insertSize,
							memory, CPUs);
					EW.flush();
					EW.close();
			...
			if (oases) {
				try {
					...
					if (!interactive) {
						sbatchFileName = oasesDir + "/scripts/" + timestamp
								+ "_" + count + "_oases.sbatch";
						generalSbatchScript.println("sbatch " + sbatchFileName);
					} else {
						sbatchFileName = oasesDir + "/scripts/" + count
								+ "_oases.sbatch";
						generalSbatchScript.println("sh " + sbatchFileName);
					}
					...
					if (interactive) {
						...
						memory = Oases.FileStart(nrOfSequences, inDir,
								trinityDir, timestamp, count, sbatch, EW, time);
					}
					Oases run = new Oases();
					run.setValues(maxKmer, minKmer, step, finalKmer, suffix,
							insertSize, projectDir, strandSpecific);
		...
		for (int i = 0; i < subDirs.size(); i++) {
			deNovoDirPE(generalSbatchScript, sbatch, timestamp, inDir + "/"
					+ subDirs.get(i), trinityDir + "/" + subDirs.get(i),
					oasesDir + "/" + subDirs.get(i));
		}
	}

	public void deNovoDirSR(ExtendedWriter generalSbatchScript,
			SBATCHinfo sbatch, String timestamp, String inDir,
			String trinityDir, String oasesDir) {
		System.out
				.println("creating de novo shell scripts for single reads in "
						+ inDir);
		...
		if (!fileNames.isEmpty() && (oases || trinity)) {
			...
			if (trinity) {
				try {
					...
					// trinitySpecific
					String sbatchFileName = trinityDir + "/scripts/"
							+ timestamp + "_" + count + "_trinity.sbatch";
					generalSbatchScript.println("sbatch " + sbatchFileName);
					ExtendedWriter EW = new ExtendedWriter(new FileWriter(
							sbatchFileName));
					String time = Trinity.getTime(nrOfSequences);
					String memory = Trinity.trinityFileStart(nrOfSequences,
							inDir, trinityDir, timestamp, count, sbatch, EW,
							time);
					int CPUs = 8;
					Trinity run = new Trinity();
					run.suffix = this.suffix;
			...
			if (oases) {
				try {
					...
					String sbatchFileName = trinityDir + "/scripts/"
							+ timestamp + "_" + count + "_oases.sbatch";
					generalSbatchScript.println("sbatch " + sbatchFileName);
					ExtendedWriter EW = new ExtendedWriter(new FileWriter(
							sbatchFileName));
					String time = Oases.getTime(nrOfSequences);
					String memory = Oases.FileStart(nrOfSequences, inDir,
							trinityDir, timestamp, count, sbatch, EW, time);
					int CPUs = 8;
					Oases run = new Oases();
					run.setValues(maxKmer, minKmer, step, finalKmer, suffix,
		...
		ArrayList<String> subDirs = IOTools.getDirectories(inDir);
		for (int i = 0; i < subDirs.size(); i++) {
			deNovoDirSR(generalSbatchScript, sbatch, timestamp, inDir + "/"
					+ subDirs.get(i), trinityDir + "/" + subDirs.get(i),
					trinityDir + "/" + subDirs.get(i));
		}
	}
