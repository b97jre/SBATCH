...
public class BWA {
		...
		if (T.containsKey("-build")) {
			...
			if (!IOTools.isDir(inDir)) {
				if (!IOTools.isDir(projectDir + "/" + inDir))
					...
					System.out.println("Neither " + inDir + " nor "
							+ projectDir + "/" + inDir + "was found");
					return;
				}
			}
			if (!IOTools.fileExists(refFile)) {
				if (!IOTools.fileExists(projectDir + "/" + refFile))
					...
					System.out.println("Neither " + refFile + " nor "
							+ projectDir + "/" + refFile + "was found");
					return;
				}
	...
	public void BWAinitiate(SBATCHinfo sbatch, String timeStamp, String inDir,
			String outDir) {
		try {
			if (!IOTools.isDir(projectDir + "/scripts"))
				IOTools.mkDir(projectDir + "/scripts");
			ExtendedWriter EW = new ExtendedWriter(new FileWriter(projectDir
					+ "/scripts/" + timeStamp + ".BWA.sh"));
			if (files) {
				BWAFile(EW, sbatch, timeStamp, outDir, forward, reverse,
						forward);
			} else {
	...
	public void BWADir(ExtendedWriter generalSbatchScript, SBATCHinfo sbatch,
			...
			try {
				...
				if (pairs.size() != 0) {
					for (int i = 0; i < pairs.size(); i++) {
						...
						BWAFile(generalSbatchScript, sbatch, timestamp, outDir,
								inDir + "/" + pairs.get(i)[0], inDir + "/"
										+ pairs.get(i)[1], readsName);
					}
				} else {
					System.out
							.println("Something wrong with the seperators? Asuming that these are single end reads");
					...
					for (int i = 0; i < fileNames.size(); i++) {
						...
						String newOutDir = outDir + "/" + fileName + "_"
								+ refName;
						BWAFile(generalSbatchScript, sbatch, timestamp,
								newOutDir, inDir + "/" + fileNames.get(i),
								null, refName);
					}
				}
			} catch (Exception E) {
		...
		for (int i = 0; i < subDirs.size(); i++) {
			BWADir(generalSbatchScript, sbatch, timestamp, inDir + "/"
					+ subDirs.get(i), outDir + "/" + subDirs.get(i));
		}

	}

	public void BWAFile(ExtendedWriter generalSbatchScript, SBATCHinfo sbatch,
			String timestamp, String outDir, String forward, String reverse,
			String readsName) {
		...
		try {
			...
			BWACommand(EW, refFile, projectDir + "/" + forward, projectDir
					+ "/" + reverse, 8, this.strandSpecifik, this.suffix,
					readsName, compress);
	...
	public static void BWACommand(ExtendedWriter EW, String refFile,
			String inFile1, String inFile2, int nrOfThreads,
			boolean strandSpecifik, String suffix, String nameSpace,
			boolean compress) {
		...
		if (inFile2 != null) {
			EW.println("bwa aln -t " + nrOfThreads + " " + refFile + " "
					+ inFile1 + "> " + refFileBase + "." + nameSpace + ".1.sai");
			EW.println("bwa aln -t " + nrOfThreads + " " + refFile + " "
					+ inFile2 + "> " + refFileBase + "." + nameSpace + ".2.sai");
			EW.println("bwa sampe " + refFile + " " + refFileBase + "."
					+ nameSpace + ".1.sai " + refFileBase + "." + nameSpace
					+ ".2.sai " + inFile1 + " " + inFile2 + " > " + refFileBase
					+ "." + nameSpace + ".pe.sam");
			if (compress) {
				SamtoolsSBATCH.sam2bam(EW, refFile + "." + nameSpace
						+ ".pe.sam", 0, 0, true, true, true, true, true);
			}
		} else {
			EW.println("bwa aln -t " + nrOfThreads + " " + refFile + " "
					+ inFile1 + "> " + refFileBase + "." + nameSpace + ".sai");
			EW.println("bwa samse " + refFile + " " + refFileBase + "."
					+ nameSpace + ".sai " + inFile1 + " > " + refFileBase + "."
					+ nameSpace + ".se.sam");
			if (compress) {
				SamtoolsSBATCH.sam2bam(EW, refFileBase + "." + nameSpace
