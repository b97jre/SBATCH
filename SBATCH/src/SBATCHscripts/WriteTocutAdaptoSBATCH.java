...
public class WriteTocutAdaptoSBATCH {
	...
	public void run(Hashtable<String, String> T) {
		...
		if (!T.containsKey("-seqPrep")) {
			...
			System.out
					.println("Add flag -6 if the phred score are ascci from 31 (hiseq-instruments)");
		} else {
			seqPrep = true;
			if (T.containsKey("-6"))
				hiseq = true;
		...
		if (!T.containsKey("-t"))
			System.out
					.println("must contain likely time  (-t 1:00:00). Time now set to default 1:00:00");

		if (T.containsKey("-a") || T.containsKey("-b")) {
			if (T.containsKey("-a")) {
				String threePrimeAdaptersFile = Functions
						.getValue(T, "-a", ".");
			...
			System.out
					.println("must contain a file that contains -3Adapter or -Adapter now only QC of sequences will be carried out.");
			cutadapt = false;
		}

		String projectDir = Functions.getValue(T, "-pDir",
				IOTools.getCurrentPath());
		String inDir = Functions.getValue(T, "-i");
		String cutAdaptDir = Functions.getValue(T, "-cutadapt", inDir
				+ "_cutadapt");
		String QCDir = Functions.getValue(T, "-QC", inDir + "_QC");
		String seqPrepDir = Functions.getValue(T, "-seqPrep", inDir
				+ "_seqPrep");
	...
	public void cutAdapt(SBATCHinfo sbatch, String projectDir, String inDir,
		...
		try {
			...
			ExtendedWriter EW = new ExtendedWriter(new FileWriter(projectDir
					+ "/scripts/" + timeStamp + "cutadapt.sh"));

			cutAdaptSample(sbatch, EW, projectDir + "/" + inDir, projectDir
					+ "/" + cutAdaptDir, projectDir + "/" + QCDir, projectDir
					+ "/" + seqPrepDir);
			EW.println();
			EW.flush();
			EW.close();
	...
	public void cutAdaptSample(SBATCHinfo sbatch,
		...
		try {
			ArrayList<String> fileNames = IOTools.getSequenceFiles(inDir,
					suffix);
			if (fileNames != null && !fileNames.isEmpty()) {
				...
				for (int i = 0; i < pairs.size(); i++) {
					String sbatchFile = finalDir + "/scripts/" + timeStamp
							+ "_" + i + "_cutadapt.sbatch";
					...
					if (cutadapt) {
						...
						for (int k = 0; k < 2; k++) {
							addCutAdaptStep(EW, inDir, cutadaptDir,
									pairs.get(i)[k]);
						}

						// QC step
						addQCstep(EW, cutadaptDir, QCDir, pairs.get(i)[0],
								pairs.get(i)[1]);
					}
					...
					if (seqPrep) {
						...
						if (hiseq)
							SP.hiseq = true;
						SP.filter_fastqSample(EW, QCDir, SeqPrepDir,
					...
					FQC.FastQCSample(EW, SeqPrepDir, pairs.get(i)[2]
							+ "merged.fastq.gz");
			...
			if (samples != null) {
				for (int i = 0; i < samples.size(); i++) {
					cutAdaptSample(sbatch, generalSbatchScript, inDir + "/"
							+ samples.get(i),
							cutadaptDir + "/" + samples.get(i), QCDir + "/"
									+ samples.get(i), SeqPrepDir + "/"
									+ samples.get(i));
				}
			}
	...
	private void addCutAdaptStep(ExtendedWriter EW, String inDir,
			String outDir, String file) {
		EW.println();
		EW.println();
		EW.println("#############################################################################################################");
	...
	private void addQCstep(ExtendedWriter EW, String inDir, String outDir,
		...
		EW.println();
		EW.println();
		EW.println("java -jar /bubo/home/h17/johanr/bin/HTStools.jar -p sequenceHandling QC -dir "
				+ inDir
				+ " -o "
				+ outDir
				+ " -l "
