...
public class Trinity {
	...
	public void run(Hashtable<String, String> T) {
		...
		sep[0] = "1." + suffix;
		sep[1] = "2." + suffix;
		int insertSize = Integer.parseInt(Functions.getValue(T, "-insertSize",
				"400"));
		...
		if (allPresent)
			if (!single)
				trinityScript(sbatch, timeStamp, inDir, outDir, pairedEnd,
						insertSize);
			else
				trinityScriptSingle(sbatch, timeStamp, inDir, outDir,
						pairedEnd, insertSize);
		else
			System.out
					.println("\n\nAborting run because of missing arguments for trinity.");
	}

	public void trinityScript(SBATCHinfo sbatch, String timeStamp,
			String inDir, String outDir, boolean PE, int instertSize) {
		try {
			if (!IOTools.isDir(projectDir + "/scripts"))
				IOTools.mkDir(projectDir + "/scripts");
			if (PE) {
				ExtendedWriter EW = new ExtendedWriter(
						new FileWriter(projectDir + "/scripts/" + timeStamp
								+ "_PE_trinity.sh"));
	...
	public void trinityScriptSingle(SBATCHinfo sbatch, String timeStamp,
			String inDir, String outDir, boolean PE, int instertSize) {
		try {
			if (!IOTools.isDir(projectDir + "/scripts"))
				IOTools.mkDir(projectDir + "/scripts");
	...
	public void trinityFilePE(ExtendedWriter EW, ArrayList<String[]> pairs,
			String outDir, int insertSize, String memory, int CPUs) {
		// Trinity.pl --seqType fq --left 13_2/QC/13_2_1.fastq --right
		// 13_2/QC/13_2_2.fastq --output
		// /bubo/proj/b2011098/private/trinity/13_2/output --min_contig_length
		// 200 --CPU 10 --bflyHeapSpace 10000M
		String catFiles = "cat ";
	...
	public void trinityFileSR(ExtendedWriter EW, ArrayList<String> files,
			String outDir, String memory, int CPUs) {
		// Trinity.pl --seqType fq --left 13_2/QC/13_2_1.fastq --right
		// 13_2/QC/13_2_2.fastq --output
		// /bubo/proj/b2011098/private/trinity/13_2/output --min_contig_length
		// 200 --CPU 10 --bflyHeapSpace 10000M
	...
	public void trinityFileSingle(ExtendedWriter EW, String file, String inDir,
			String outDir) {
		// Trinity.pl --seqType fq --left 13_2/QC/13_2_1.fastq --right
		// 13_2/QC/13_2_2.fastq --output
		// /bubo/proj/b2011098/private/trinity/13_2/output --min_contig_length
		...
		trinityFileEndSingle(EW, trinitycommand,
				outDir + "/" + file.substring(0, file.indexOf(this.suffix) - 1));
	}
	...
	public static String trinityFileStart(int nrOfSequences, String inDir,
			String outDir, String timestamp, int count, SBATCHinfo sbatch,
			ExtendedWriter EW, String time) {
		...
		if (time.compareTo("00.00.00") == 0) {
			System.out.println("time not set will be set to " + hours
					+ " hours");
			newTime = hours + ":00:00";
		} else {
			newTime = time;
		...
		if (nrofMB < 3) {
			...
			sbatch.printSBATCHinfoCore(EW, outDir, timestamp, count, "trinity_"
					+ split1[split1.length - 1], newTime);
			memory = "2G";
		} else if (nrofMB < 24) {
			System.out.println("Memory allocated will be 24 GB");
			sbatch.printSBATCHinfo(EW, outDir, timestamp, count, "trinity_"
					+ split1[split1.length - 1], newTime);
			memory = "23G";
		} else if (nrofMB < 36) {
			System.out.println("Memory allocated will be 36 GB");
			sbatch.printSBATCHinfoFat(EW, outDir, timestamp, count, "trinity_"
					+ split1[split1.length - 1], newTime);
			memory = "34G";
		} else if (nrofMB < 72) {
			System.out.println("Memory allocated will be 72 GB");
			sbatch.printSBATCHinfo72GB(EW, outDir, timestamp, count, "trinity_"
					+ split1[split1.length - 1], newTime);
			memory = "70G";
		} else {
			sbatch.printSBATCHinfohalvan(EW, outDir, timestamp, count,
					"trinity_" + split1[split1.length - 1], newTime, nrofMB);
			memory = "72G";
		}
	...
	public void trinityFileStartSingle(String inDir, String outDir,
			String timestamp, int count, SBATCHinfo sbatch, ExtendedWriter EW) {
	...
	public void trinityFileEND(ExtendedWriter EW, String trinitycommand,
			String outDir, String memory, int CPUs) {
	...
	public void trinityFileEndSingle(ExtendedWriter EW, String trinitycommand,
			String outDir) {
		trinitycommand += " --output " + outDir;
	...
	public void trinityDirPE(ExtendedWriter generalSbatchScript,
		...
		if (fileNames.isEmpty()) {
			...
			try {
				...
				int nrOfSequences = FastQSequences.countSequencesPE(inDir,
						pairs);
				String sbatchFileName = outDir + "/scripts/" + timestamp + "_"
						+ count + "_trinity.sbatch";
				generalSbatchScript.println("sbatch " + sbatchFileName);

				ExtendedWriter EW = new ExtendedWriter(new FileWriter(
						sbatchFileName));

				String memory = trinityFileStart(nrOfSequences, inDir, outDir,
						timestamp, count, sbatch, EW, time);
				int CPUs = 8;
		...
		for (int i = 0; i < subDirs.size(); i++) {
			trinityDirPE(generalSbatchScript, sbatch, timestamp, inDir + "/"
					+ subDirs.get(i), outDir + "/" + subDirs.get(i), insertSize);
		}
	}

	public void trinityDirSR(ExtendedWriter generalSbatchScript,
			SBATCHinfo sbatch, String timestamp, String inDir, String outDir) {
		...
		if (fileNames.isEmpty()) {
			...
			try {
				...
				int nrOfSequences = FastQSequences.countSequencesSR(inDir,
						fileNames);
		...
		ArrayList<String> subDirs = IOTools.getDirectories(inDir);
		for (int i = 0; i < subDirs.size(); i++) {
			trinityDirSR(generalSbatchScript, sbatch, timestamp, inDir + "/"
					+ subDirs.get(i), outDir + "/" + subDirs.get(i));
		}
	}

	public void trinityDirSRSingle(ExtendedWriter generalSbatchScript,
			SBATCHinfo sbatch, String timestamp, String inDir, String outDir) {
			...
			while (count < fileNames.size() / perRun + 1) {
				try {
					String sbatchFileName = outDir + "/scripts/" + timestamp
							+ "_" + count + "_trinity.sbatch";
					generalSbatchScript.println("sbatch " + sbatchFileName);

					ExtendedWriter EW = new ExtendedWriter(new FileWriter(
							sbatchFileName));

					trinityFileStartSingle(inDir, outDir, timestamp, count,
							sbatch, EW);

					for (int i = 0; i < perRun; i++) {
						if (number < fileNames.size()) {
							EW.println();
							trinityFileSingle(EW, fileNames.get(number), inDir,
									outDir);
							number++;
						}
					}
