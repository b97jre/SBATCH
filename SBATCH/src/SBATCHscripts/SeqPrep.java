...
public class SeqPrep {
	...
	public void filterFasta(String projectDir, String inDir, String outDir) {
		try {
			...
			filter_fastqSample(EW, projectDir + "/" + inDir, projectDir + "/"
					+ outDir);
			EW.flush();
			EW.close();
		} catch (Exception E) {
	...
	public void filter_fastqSample(ExtendedWriter generalSbatchScript,
		...
		for (int i = 0; i < samples.size(); i++) {
			filter_fastqSample(generalSbatchScript,
					inDir + "/" + samples.get(i), outDir + "/" + samples.get(i));
		}
		...
		if (fileNames.isEmpty()) {
			...
			try {
				...
				for (int i = 0; i < pairs.size(); i++) {
					...
					if (hiseq)
						EW.print(" -6 ");
					EW.print(" -f " + pairs.get(i)[0] + " -r "
							+ pairs.get(i)[1]);
					EW.print(" -1 " + outDir + "/" + pairs.get(i)[0]
							+ ".gz -2 " + outDir + "/" + pairs.get(i)[1]
							+ ".gz");
					EW.println(" -s " + outDir + "/" + pairs.get(i)[2]
							+ "merged.fastq.gz");
					EW.println();
					EW.flush();
					EW.close();
	...
	public void filter_fastqSample(ExtendedWriter EW, String inDir,
		...
		EW.print(" -1 " + outDir + "/" + forward + ".gz -2 " + outDir + "/"
				+ reverse + ".gz");
