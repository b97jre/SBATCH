...
public class GATK {
	...
	public void GATKInitial(SBATCHinfo sbatch, String timeStamp, String inDir,
		...
		try {
			...
			if (T.containsKey("-Genotype")) {
				...
				ExtendedWriter EW = new ExtendedWriter(new FileWriter(
						projectDir + "/scripts/" + timeStamp + "." + lastDir
								+ ".GATK.GATKGenotypeAndValidate.sh"));
	...
	public void GATKPhaseSNPs(ExtendedWriter generalSbatchScript,
		...
		if (!fileNames.isEmpty()) {
			try {
				GATKPhaseSNPsSample(generalSbatchScript, sbatch, timestamp,
						inDir, fileNames, T);
			} catch (Exception E) {
				E.printStackTrace();
			}
		...
		for (int i = 0; i < subDirs.size(); i++) {
			GATKPhaseSNPs(generalSbatchScript, sbatch, timestamp, inDir + "/"
					+ subDirs.get(i), T);
		}
	}

	public void GATKGenotype(ExtendedWriter generalSbatchScript,
			SBATCHinfo sbatch, String timestamp, String inDir,
			Hashtable<String, String> T) {
		...
		if (!fileNames.isEmpty()) {
			try {
				GATKGenotypeSample(generalSbatchScript, sbatch, timestamp,
						inDir, fileNames, T);
			} catch (Exception E) {
				E.printStackTrace();
			}
		...
		ArrayList<String> subDirs = IOTools.getDirectories(inDir);
		for (int i = 0; i < subDirs.size(); i++) {
			GATKGenotype(generalSbatchScript, sbatch, timestamp, inDir + "/"
					+ subDirs.get(i), T);
		}
	}

	public void GATKDirPhase1(ExtendedWriter generalSbatchScript,
			SBATCHinfo sbatch, String timestamp, String inDir,
			Hashtable<String, String> T) {
		...
		if (!fileNames.isEmpty()) {
			try {
				GATKPhase1Sample(generalSbatchScript, sbatch, timestamp, inDir,
						fileNames, T);
			} catch (Exception E) {
				E.printStackTrace();
			}
		...
		ArrayList<String> subDirs = IOTools.getDirectories(inDir);
		for (int i = 0; i < subDirs.size(); i++) {
			GATKDirPhase1(generalSbatchScript, sbatch, timestamp, inDir + "/"
					+ subDirs.get(i), T);
		}
	}
	...
	public static void printSBATCHscript(ExtendedWriter MasterShellScriptFile,
			SBATCHinfo sbatch, String timestamp, int count, String outDir,
			String commandLine, String job, String outFile, String function,
			String time) {
		try {
			String sbatchFileName = outDir + "/scripts/" + timestamp + "_"
					+ count + "_" + function + ".sbatch";
			...
			if (job.compareTo("thin") == 0)
				sbatch.printSBATCHinfo(EW, outDir, timestamp, count, function,
						time);
			else if (job.compareTo("core") == 0)
				sbatch.printSBATCHinfoCore(EW, outDir, timestamp, count,
						function, time);
			else if (job.compareTo("fat") == 0)
				sbatch.printSBATCHinfoFat(EW, outDir, timestamp, count,
						function, time);
			else
				sbatch.printSBATCHinfo(EW, outDir, timestamp, count, function,
						time);
					...
					while (stop < size) {
						...
						if (!rerun
							...
							printSBATCHscript(MasterShellScriptFile, sbatch,
									timestamp, count, outDir, commandLine, job,
									outFile, "GATK_phase2", time);
						start = stop - 2 * padding;
						stop = stop + cutoff;
						count++;
					}
	...
	public void GATKMerge(ExtendedWriter MasterShellScriptFile,
			SBATCHinfo sbatch, String timestamp, String outDir,
			Hashtable<String, String> T) {
		...
		if (!IOTools.isDir(outDir + "/scripts"))
			IOTools.mkDir(outDir + "/scripts");
		try {
			ArrayList<String> vcfFiles = IOTools.getSequenceFiles(outDir,
					suffix);
			...
			System.out.println();
			System.out.println();
			String commandLine = mergeVCFfiles(memory, GATKdir, vcfFiles,
					Reference, prefix + ".final.vcf.temp", this.PRIORITIZE, 8);
			printSBATCHscript(MasterShellScriptFile, sbatch, timestamp, 0,
					outDir, commandLine, job, prefix + ".final.vcf",
					"GATK_mergeVCF", time);
	...
	public void GATKPhase1Sample(ExtendedWriter generalSbatchScript,
			SBATCHinfo sbatch, String timestamp, String outDir,
		...
		try {
			...
			if (T.containsKey("-ReAlign")
				...
				for (int i = 0; i < fileNames.size(); i++) {
					...
					String sbatchFileName = outDir + "/scripts/" + timestamp
							+ "_" + i + "_phase1_GATK.sbatch";
					generalSbatchScript.println("sbatch " + sbatchFileName);
					ExtendedWriter EW = new ExtendedWriter(new FileWriter(
							sbatchFileName));
					sbatch.printSBATCHinfo(EW, outDir, timestamp, 0,
							"GATK_phase1", time);
					EW.println();
					EW.println("cd " + outDir);
					...
					if (T.containsKey("-ReassignOneMappingQuality")) {
						bamFile = ReassignOneMappingQuality(EW, bamFile,
								memory, this.GATKdir, this.Reference, suffix);
					}
					if (T.containsKey("-ReAlign"))
						bamFile = ReAlign(EW, bamFile, memory, this.GATKdir,
								this.Reference, suffix, knownSNPVCF,
					...
					if (T.containsKey("-BQSRprint"))
						bamFile = BQSRprint(EW, bamFile, memory,
								this.picardDir, this.GATKdir, this.Reference,
								suffix, knownSNPVCF, knownIndelVCF);
			...
			if (phase1) { // run the entire pipeline
				if (T.containsKey("-preGATK")) {
					...
					// marking duplicates
					String bamFile = Picard.preGATK(EW, sbatch, timestamp,
							outDir, fileNames, T, this.picardDir, this.memory,
							this.suffix, this.nucleicAcid);

					// Changes suffix from sam to bam
					if (suffix.lastIndexOf("sam") > -1) {
						this.suffix = this.suffix.substring(0,
								suffix.lastIndexOf("sam"))
								+ "bam";
					}
					...
					// takes care of realining and recalibration
					phase1(EW, bamFile, memory, this.picardDir, this.GATKdir,
					...
					for (int i = 0; i < fileNames.size(); i++) {
						String sbatchFileName = outDir + "/scripts/"
								+ timestamp + "_" + i + "_phase1_GATK.sbatch";
						generalSbatchScript.println("sbatch " + sbatchFileName);
						ExtendedWriter EW = new ExtendedWriter(new FileWriter(
								sbatchFileName));
						sbatch.printSBATCHinfo(EW, outDir, timestamp, 0,
						...
						phase1(EW, bamFile, memory, this.picardDir,
	...
	public static String phase1(ExtendedWriter EW, String bamFile, int memory,
			String picardDir, String GATKDir, String Reference, String suffix,
		...
		if (ReassignOneMappingQuality)
			bamFile = ReassignOneMappingQuality(EW, bamFile, memory, GATKDir,
					Reference, suffix);
		bamFile = ReAlign(EW, bamFile, memory, GATKDir, Reference, suffix,
				knownSNPVCF, knownIndelsVCF, targetIntervals);
		bamFile = BQSR(EW, bamFile, memory, GATKDir, Reference, suffix,
	...
	public static String BQSRprint(ExtendedWriter EW, String bamFile,
			int memory, String picardDir, String GATKDir, String Reference,
			String suffix, String knownSNPVCF, String knownIndelsVCF) {
		String baseName = bamFile.substring(0, bamFile.indexOf(".bam"));
		EW.println("# BaseQualityRecalibration  ");
	...
	public static String ReadBackedPhasing(int memory, String GATKDir,
			String bamFile, String reference, String outFileVCF,
		...
		String commandLine = "java -Xmx" + memory + "g -jar " + GATKDir
				+ "/GenomeAnalysisTK.jar " + "-R " + reference + " "
				+ "-T ReadBackedPhasing " + "--variant  " + knownSNPVCF + " "
				+ "-L  " + knownSNPVCF + " " + "-o " + outFileVCF
	...
	public static String mergeVCFfiles(int memory, String GATKDir,
			ArrayList<String> vcfFiles, String reference, String outFileVCF,
			boolean PRIORITIZE, int cores) {
		/*
		 * java -Xmx2g -jar GenomeAnalysisTK.jar \ -R ref.fasta \ -T
		 * CombineVariants \ --variant input1.vcf \ --variant input2.vcf \ -o
		 * output.vcf \ -genotypeMergeOptions UNIQUIFY
		 * 
		 * java -Xmx2g -jar GenomeAnalysisTK.jar \ -R ref.fasta \ -T
		 * CombineVariants \ --variant:foo input1.vcf \ --variant:bar input2.vcf
		 * \ -o output.vcf \ -genotypeMergeOptions PRIORITIZE -priority foo,bar
		 */

		String commandLine = "java -Xmx" + memory + "g -jar " + GATKDir
				+ "/GenomeAnalysisTK.jar " + "-R " + reference + " "
				+ "-T CombineVariants " + "-o " + outFileVCF + " -nt " + cores;
