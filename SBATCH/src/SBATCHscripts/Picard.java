...
public class Picard {
	...
	public void run(Hashtable<String, String> T) {
		...
		if (!T.containsKey("-picardDir"))
			System.out
					.println("must contain directory where picard jar files are kept. -picardDir  now set to default /bubo/sw/apps/bioinfo/picard/1.92/kalkyl/");
	...
	public void PicardInitial(SBATCHinfo sbatch, String timeStamp,
			String inDir, Hashtable<String, String> T) {
		try {
			...
			ExtendedWriter EW = new ExtendedWriter(new FileWriter(projectDir
					+ "/scripts/" + timeStamp + "." + lastDir + ".Picard.sh"));
	...
	public void PicardDir(ExtendedWriter generalSbatchScript,
			SBATCHinfo sbatch, String timestamp, String inDir,
		...
		if (!fileNames.isEmpty()) {
			try {
				...
				for (int i = 0; i < fileNames.size(); i++) {
					...
					PicardFile(generalSbatchScript, sbatch, timestamp, inDir,
							fileNames.get(i), T, i);
				}
			} catch (Exception E) {
				E.printStackTrace();
		...
		for (int i = 0; i < subDirs.size(); i++) {
			PicardDir(generalSbatchScript, sbatch, timestamp, inDir + "/"
					+ subDirs.get(i), T);
		}
	}

	public static String preGATK(ExtendedWriter EW, SBATCHinfo sbatch,
			String timestamp, String outDir, ArrayList<String> fileNames,
			Hashtable<String, String> T, String picardDir, int memory,
			String suffix, String Nucleotide) {
		...
		for (int i = 0; i < fileNames.size(); i++) {
			...
			String RGID = Functions.getValue(T, "-RGID", RGSM + "_" + RGPU
					+ "_" + RGLB + "_" + Nucleotide + "_" + Lane);
		...
		if (fileNames.size() > 1) {
			fileName = Functions.getCommonPrefix(fileNames.get(0),
					fileNames.get(1));

			EW.println("# merging all bamFiles into one file named " + fileName
					+ ".AddOrReplaceReadGroups.merged.bam");
			bamFile = mergeBamFiles(EW, memory, picardDir, fileNames, fileName
					+ ".AddOrReplaceReadGroups.merged.bam");
			EW.println();
			EW.println();
		} else {
	...
	public static String preGATKDNAsample(ExtendedWriter EW, SBATCHinfo sbatch,
			String timestamp, String outDir, String fileName,
			Hashtable<String, String> T, String picardDir, int memory,
			String suffix) {
		...
		String RGPU = Functions.getValue(T, "-RGPU", Barcode);
		String RGSM = Functions.getValue(T, "-RGSM", Name);
		String RGID = Functions.getValue(T, "-RGID", RGSM + "_" + RGPU + "_"
				+ RGLB + "_" + Nucleotide + "_" + Lane);

		bamFile = AddOrReplaceReadGroups(EW, bamFile, memory, picardDir,
				suffix, RGID, RGLB, RGPL, RGPU, RGSM, "coordinate")
				+ "."
				+ suffix;
		EW.println("# Marking duplicates duplicates " + bamFile);
	...
	public void PicardFile(ExtendedWriter generalSbatchScript,
			SBATCHinfo sbatch, String timestamp, String outDir, String bamFile,
			Hashtable<String, String> T, int count) {
		...
		try {
			...
			if (T.containsKey("-SortSam")) {
				String sortOrder = Functions.getValue(T, "-SORT_ORDER",
						"coordinate");
				bamFile = SortSam(EW, bamFile, memory, picardDir, suffix,
						sortOrder) + "." + suffix;
			}
			if (T.containsKey("-preGATK")) {
				System.out
						.println("Asumes that the sam filename has the following order RGSM_DNA_RGPU_RGLB_WHATEVER for DNA or");
				System.out
						.println("Intra8_2.Replicate.Tissue.RGPU.RGLB.Cr_reference.bam for RNAs");
				...
				if (T.containsKey("-RNA")) {
					...
					String RGID = Functions.getValue(T, "-RGID", RGSM + "_"
							+ Replicate + "_" + RGPU + "_" + RGLB + "_RNA_"
							+ Lane + "_" + Tissue);

					prepareForGATK(EW, bamFile, memory, picardDir, suffix,
							RGID, RGLB, RGPL, RGPU, RGSM, "coordinate");
				} else {
	...
	public static String prepareForGATK(ExtendedWriter EW, String bamFile,
			int memory, String jarDir, String suffix, String RGID, String RGLB,
			String RGPL, String RGPU, String RGSM, String sortOrder) {
		...
		bamFile = AddOrReplaceReadGroups(EW, bamFile, memory, jarDir, suffix,
				RGID, RGLB, RGPL, RGPU, RGSM, sortOrder) + "." + suffix;
	...
	public static String AddOrReplaceReadGroups(ExtendedWriter EW,
			String inFile1, int memory, String jarDir, String suffix,
			String RGID, String RGLB, String RGPL, String RGPU, String RGSM,
			String sortOrder) {

		String baseFile = inFile1.substring(0, inFile1.lastIndexOf(suffix) - 1)
		...
		String AddOrReplaceReadGroupsCommand = "java -Xmx" + memory + "G -jar " + jarDir
				...
				+ baseFile + "." + suffix + " RGID=" + RGID + " " + // String
																	...
																	// default
																	// value.
				"RGLB=" + RGLB + " " + // String Read Group Library Required.
										// Insert size
				"RGPL=" + RGPL + " " + // String Read Group platform (e.g.
										// Illumina, solid) Required.
				"RGPU=" + RGPU + " " + // String Read Group platform unit (eg.
										// run barcode) Required.
				"RGSM=" + RGSM + " " + // String Read Group sample name
										// Required.
				"VALIDATION_STRINGENCY=LENIENT SORT_ORDER=" + sortOrder; // Optional
																			// sort
																			// order
																			// to
																			// output
	...
	public static String markDuplicates(ExtendedWriter EW, String inFile1,
			int memory, String jarDir, String suffix) {

		String baseFile = inFile1.substring(0, inFile1.lastIndexOf(suffix) - 1)
				+ ".markDuplicates";
		EW.println();
		String bowtiecommand = "java -Xmx" + memory + "G -jar " + jarDir
				+ "/MarkDuplicates.jar INPUT=" + inFile1 + " OUTPUT="
				+ baseFile + "." + suffix + " METRICS_FILE=" + baseFile
				+ ".metrics VALIDATION_STRINGENCY=LENIENT";
		EW.println(bowtiecommand);
		EW.println();
		return baseFile + "." + suffix;
	}

	public static String SortSam(ExtendedWriter EW, String inFile1, int memory,
			String jarDir, String suffix, String sortOrder) {

		String baseFile = inFile1.substring(0, inFile1.lastIndexOf(suffix) - 1)
				+ ".SortSam";
		EW.println();
		String bowtiecommand = "java -Xmx" + memory + "G -jar " + jarDir
				+ "/SortSam.jar INPUT=" + inFile1 + " OUTPUT=" + baseFile + "."
				+ suffix + " SORT_ORDER=" + sortOrder
	...
	public static void CreateSequenceDictionary(ExtendedWriter EW,
			String jarDir, int memory, String inFile1, String outFile) {

		EW.println();
		// java -jar CreateSequenceDictionary.jar R=
		// Homo_sapiens_assembly18.fasta O= Homo_sapiens_assembly18.dict - See
		// more at:
		// http://gatkforums.broadinstitute.org/discussion/1601/how-can-i-prepare-a-fasta-file-to-use-as-reference#latest

		String bowtiecommand = "java -Xmx" + memory + "G -jar " + jarDir
				+ "/CreateSequenceDictionary.jar  R= " + inFile1 + " O= "
				+ outFile;
		EW.println(bowtiecommand);
		EW.println();
	...
	public static String mergeBamFiles(ExtendedWriter EW, int memory,
			String jarDir, ArrayList<String> fileNames, String mergedFileName) {
		String commandLine = "java -Xmx" + memory + "G -jar " + jarDir
				+ "/MergeSamFiles.jar OUTPUT=" + mergedFileName + " ";
		for (int i = 0; i < fileNames.size(); i++) {
