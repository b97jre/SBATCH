...
public class TopHatSBATCH {
	...
	public void run(Hashtable<String, String> T) {
		...
		innerDistanceSTD = Integer.parseInt(Functions
				.getValue(T, "-rSTD", "70"));
	...
	public void tophat(SBATCHinfo sbatch, String timeStamp, String inDir,
			String outDir) {
		try {
	...
	public void tophatPair(ExtendedWriter generalSbatchScript,
			SBATCHinfo sbatch, String timestamp, String inDir, String outDir,
			int count, String right, String left) {
		String sbatchFileName = outDir + "/scripts/" + timestamp + "_" + count
				+ "_tophat.sbatch";
		...
		try {
			...
			EW.println();
			EW.print("tophat -o " + outDir + "/pair_" + count + "  -r "
					+ this.innerDistance + " --mate-std-dev "
					+ this.innerDistanceSTD + " " + this.referenceFile);
			EW.print(" " + right);
			EW.print(" " + left);
			EW.println();
			EW.flush();
	...
	public void tophatDir(ExtendedWriter generalSbatchScript,
			SBATCHinfo sbatch, String timestamp, String inDir, String outDir) {
		...
		if (fileNames.isEmpty()) {
			...
			if (!pairs.isEmpty()) {
				for (int i = 0; i < pairs.size(); i++) {
					tophatPair(generalSbatchScript, sbatch, timestamp, inDir,
							outDir, i, pairs.get(i)[0], pairs.get(i)[1]);
				}
			}
		}
		ArrayList<String> subDirs = IOTools.getDirectories(inDir);
		for (int i = 0; i < subDirs.size(); i++) {
			tophatDir(generalSbatchScript, sbatch, timestamp, inDir + "/"
